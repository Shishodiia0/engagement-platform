...
from backend.config import ETL_INTERVAL_MINUTES


def run_etl():
    now = datetime.now(timezone.utc)
    print(f"[ETL] Starting at {now}")
    try:
        ...
        print(f"[ETL] Done. Rows synced: {total_rows}")
...
if __name__ == "__main__":
    ...
    scheduler.add_job(run_etl, "interval", minutes=ETL_INTERVAL_MINUTES)
    print(f"[ETL] Scheduler started. Running every {ETL_INTERVAL_MINUTES} minutes.")
    run_etl()
    scheduler.start()
